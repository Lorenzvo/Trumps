// Landing screen: create a room or join one by code. Reuses the pill-btn/panel/
// badge-pixel utility classes from game/TwoPlayerGame.css (loaded globally since
// App.tsx always imports that module) rather than redefining them here.
...
export function MainMenu({
  ...
  async function handleJoin() {
    ...
    setError(null)
    try {
      await joinRoom(joinCode.trim(), name.trim(), clientId)
      onEntered(joinCode.trim())
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err))
    } finally {
      setBusy(false)
      ...
      <div className="menu-columns">
        ...
        <section className="panel menu-card">
          ...
          <input
            ...
            maxLength={5}
          />
          <button type="button" className="pill-btn secondary" onClick={handleJoin} disabled={busy}>
            Join room
          </button>
        </section>
      </div>
